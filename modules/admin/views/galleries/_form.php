...
<div class="gallery-galleries-form">
    ...
    <div class="nav-tabs-custom">
        <ul class="nav nav-tabs">
            <li class="active"><a href="#tab_setting" data-toggle="tab" aria-expanded="true">Setting</a></li>
            <li class=""><a href="#tab_images" data-toggle="tab" aria-expanded="true">Images</a></li>
        </ul>
        ...
        <div class="tab-content">
                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane" id="tab_hover_effects">
                            ...
                            ?>
                                ...
                                <hr style="margin-top: 7px;margin-bottom: 3px;padding: 0;">
                            <?php
                                $i++;
                            }
            ...
            <div class="tab-pane" id="tab_images">
                <?php $form = ActiveForm::begin(['options' => ['id' => 'form-add-file', 'enctype' => 'multipart/form-data'], 'action' => ['/admin/file/upload']]);
                $model_fies = new \app\modules\admin\models\UploadFiles();
                ?>

                    <?= $form->field($model_fies, 'imageFiles[]')->fileInput(['multiple' => true, 'accept' => 'file/*']) ?>

                    <button id="image-conversation-button" type="button" class="btn btn-block btn-lg btn-default">Upload</button>

                <?php ActiveForm::end() ?>

                <ul class="list-group container-fluid" style="padding-left: 15px; margin-top: 15px;">
                    <?php foreach ($model->images as $item){ ?>
                        <li id="row-image-<?= $item->id ?>" class="list-group-item row" style="text-align: center">
                            <div class="col-md-2">
                                <img class="" src="/files/galleries/gallery_<?= $model->id ?>/<?= $item->path ?>" alt="<?= $item->title ?>" style="max-height: 150px;width: auto;max-width: 100%">
                            </div>
                            <div class="col-md-9">
                                <?php $form = ActiveForm::begin(['action' => ['/galleries/gallery/ajax-update-title-description'], 'options' => ['class' => 'form-image']]); ?>

                                    <?= $form->field($item, 'title')->textInput() ?>
                                    <?= $form->field($item, 'description')->textInput() ?>
                                    <div class="hidden">
                                        <?= $form->field($item, 'id')->hiddenInput()->label('') ?>
                                    </div>

                                    <div id="alert-form-image-<?= $item->id ?>" style="z-index:-1;transition: all 1s;position: absolute;width: 100%;height: 100%;top: 0;left: 0;display : flex;align-items: center;justify-content: center;opacity: 0;">
                                        <div class="alert alert-success" role="alert">
                                            Зміни успішно збережено!!!
                                        </div>
                                    </div>

                                    <button type="submit" class="btn btn-block btn-lg btn-default image-conversation-button">Save</button>

                                <?php ActiveForm::end() ?>

                            </div>
                            <div class="col-md-1">
                                <!--<button type="button" class="btn btn-success image-up" data-image-id="< ?= $item->id ?>"><i class="fa fa-arrow-up"></i></button> <br>
                                <button type="button" class="btn btn-success image-down" data-image-id="< ?= $item->id ?>"><i class="fa fa-arrow-down"></i></button> <br> -->
                                <button type="button" class="btn btn-danger image-delete" data-delete-image="<?= $item->id ?>"><i class="fa fa-close"></i></button>
                            </div>
                        </li>
                    <?php } ?>
                </ul></div>
...
JS;
$this->registerJsVar('url_upload',  Url::toRoute('/admin/file/ajax-upload/'.$model->id, true));
$this->registerJs($script);
?>
...
<?php
$script =  <<< JS
    $(document).on("beforeSubmit", ".form-image", function () {
        var form = $(this);
        
        // submit form
        $.ajax({
            url    : '/galleries/gallery/ajax-update-title-description', //form.attr('action'),
            type   : 'post',
            data   : form.serialize(),
            success: function (response) {
                if(response){
                    $("#alert-form-image-"+response).css("opacity", "1");
                    $("#alert-form-image-"+response).css("z-index", "100");
                    
                    setTimeout(function() { 
                        $("#alert-form-image-"+response).css("opacity", "0"); 
                        $("#alert-form-image-"+response).css("z-index", "-1");
                    }, 2000);
                }
            },
            error  : function () {
...
$script =  <<< JS
     $('.image-delete').on( 'click', function( event ){     
        $.ajax({
            url         : '/galleries/gallery/ajax-delete-image',
            type        : 'POST',
            data        : {
                image:  this.getAttribute('data-delete-image'),
            },
                success: function (data) {
                //console.log(data.message);
                $('#row-image-'+data).remove();
            }    
        });
    });
     ...
     /*$('.image-down').on( 'click', function( event ){         
        $.ajax({
            url         : '/galleries/gallery/ajax-down-image',
            type        : 'POST',
            data        : {
                image:  this.getAttribute('data-image-id'),
            },
            success: function (data) {
                alert(data);
            }    
     ...
     $('.image-up').on( 'click', function( event ){         
        $.ajax({
            url         : '/galleries/gallery/ajax-up-image',
            type        : 'POST',
            data        : {
                image:  this.getAttribute('data-image-id'),
            },
